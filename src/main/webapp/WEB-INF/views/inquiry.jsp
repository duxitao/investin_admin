...
<body style="width: 780px;">
	...
	<form id="formID" action="inquiry" method="post">
		<div class="row-fluid">
					...
					<c:forEach items="${list}" var="info" varStatus="status">
						<tr>
							...
							<td><a id="translate" href="javascript:void(0);"
								role="${info.id}">翻译</a><c:if test="${info.status!='4'}">（新）</c:if></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
		...
		<script type="text/javascript">
			...
			window.onload = function() {
				/*
				$(window.parent.document).find("#small_project").load(function () {
				    var main = $(window.parent.document).find("#small_project");
